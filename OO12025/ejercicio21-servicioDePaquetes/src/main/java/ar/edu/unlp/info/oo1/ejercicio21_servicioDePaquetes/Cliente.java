...
public abstract class Cliente {
	...
	public Cliente(String n, String d) {
		this.setDireccion(d);
		this.setNombre(n);
		this.setEnvios(new LinkedList<Envio>());
	}


	public void agregarEnvios(Envio e) {
		this.envios.add(e);
	}
	
	public double montoAPagar(DateLapse periodo) {
		return this.envios.stream().filter(fecha -> periodo.includesDate(fecha.getFecha())).mapToDouble(monto -> this.montoAPagarConDescuento(monto.calcularCosto())).sum();
	}
